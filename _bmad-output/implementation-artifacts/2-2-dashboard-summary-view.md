...
- [x] [Review][Patch][Decision resolved: exclude stale teams] Company-wide Achievement %/Growth % aggregation blends each team's most-recent `sales_data` row without verifying all teams' "latest" rows share the same operational date — if one team's nightly row is missing or rejected on a given night, the headline figure silently mixes data from different days. Fixed: `_aggregate_company_wide` now excludes any team whose latest row's `date` is older than the most recent date represented among the fetched rows, so the weighted average only reflects one consistent operational day; the team still appears in the Team Performance breakdown. Verified via new `test_aggregate_company_wide_excludes_teams_whose_latest_row_predates_the_most_recent_date`. [domain/metrics.py:42-71,87-123]
- [x] [Review][Patch][Decision resolved: reserve min-height] Team Performance tile's loading skeleton is a fixed 38px block (StatTile's generic skeleton) but its loaded content is one row per team — with more than ~1 team, the tile visibly grows once data resolves, contradicting Task 5's explicit "tile dimensions don't jump between loading and loaded" design note. Fixed: added an optional `StatTile.skeletonHeight` prop; the Team Performance tile now reserves 140px, reducing (not eliminating) the jump for typical team counts. Verified via new `StatTile.test.tsx` case. [web/src/components/StatTile.tsx:26-27, web/src/pages/DashboardPage.tsx:299-320]
- [x] [Review][Patch] On `/dashboard/summary` fetch failure, the freshness badge and all seven `StatTile`s stay permanently stuck on "Loading…"/skeleton state even though the error `Alert` with Retry is shown — an internally contradictory UI (visible error banner alongside tiles that claim to still be loading indefinitely). Fixed: `loading` now excludes the error case, tiles render `'—'` instead of a skeleton, and the freshness badge shows a neutral-but-distinct "Unable to load" state. Verified via an extended assertion on the existing "shows an error banner" test. [web/src/pages/DashboardPage.tsx:212-217]
- [x] [Review][Patch] Growth-trend arrow/color direction is `Math.round(Number(growth_pct)) >= 0 ? 'up' : 'down'` — for any `growth_pct` in (-0.5, 0) (e.g. `"-0.4"`), `Math.round` yields `-0`, and JS's `-0 >= 0` evaluates `true`, so small negative growth renders the green "up" arrow next to a "0%" label. Fixed: direction is now derived from the raw value's sign (`Number(growth_pct) < 0`), not the rounded display value. Verified via new `DashboardPage.test.tsx` case (`growth_pct: '-0.4'` renders the error/down badge). [web/src/pages/DashboardPage.tsx:277-289]
- [x] [Review][Patch] `tests/api/test_dashboard_routes.py` seeds `sales_data.date` using the exact same `datetime.now(UTC).astimezone(ZoneInfo("Asia/Dhaka")).date()` expression the route handler itself uses to compute "today" — the test can never catch a timezone-conversion regression in the route, since both sides independently recompute the same (possibly wrong) value rather than the test asserting against an independently-derived expectation. Fixed: seeds a fixed `date`/`datetime` pair chosen at the UTC/Dhaka day boundary (20:00 UTC = 02:00 next day in Dhaka) and monkeypatches `api.dashboard.routes.datetime.now` to that fixed instant, so the test's expectation is independent of the route's own computation. [tests/api/test_dashboard_routes.py:15]
- [x] [Review][Patch] `formatCrBdt`/`formatPercent` call `Number(...)` on the API's string input with no NaN guard — a malformed or unexpected value renders as the literal string "NaN Cr"/"NaN%" instead of a safe fallback. Fixed: both return `'—'` on a non-finite parse. Verified via two new `format.test.ts` cases. [web/src/utils/format.ts:5-12]
- [x] [Review][Patch] `response.json()` is cast directly to `DashboardSummary` with no runtime validation — a contract-drifted 200 response (e.g. a missing `team_performance` field) would crash the page (`.map` on `undefined`) instead of surfacing the existing error banner. Fixed: the parsed body is now checked for a `team_performance` array before use; anything else routes to the existing error-banner state instead of crashing. [web/src/pages/DashboardPage.tsx:132]
- [x] [Review][Patch] `DashboardPage.test.tsx` does not port `HomePage.test.tsx`'s "redirects to / when the /auth/me request itself fails" (network-rejection) case — the session-guard effect's `.catch()` network-failure-redirect branch has zero regression coverage in the new file. Fixed: ported the test case verbatim. [web/src/pages/DashboardPage.test.tsx]
- [x] [Review][Defer] `list_all()`/`latest_per_team()` (adapters/persistence/teams.py, adapters/persistence/sales_data.py) don't filter to active-only teams, and an active + soft-deleted team can share the same display name, causing a React key collision in the Team Performance tile — deferred, pre-existing: `Team.status`/soft-delete didn't exist at this story's commit (b8fb965..7aad716); it was added afterward by Story 3.1, which never updated these Dashboard read paths to match its own `get_by_name`/`get_or_create_by_name` active-only convention. Real in current HEAD; needs a follow-up against Story 3.1's integration, not this story. [adapters/persistence/teams.py:80-83, adapters/persistence/sales_data.py:84-91]
